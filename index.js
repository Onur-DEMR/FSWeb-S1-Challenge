...
let metin= [index]". indekste bulunan fenomen: " + fenomenler[index].profile ;
return metin;
}
...
function fenomenEkle(fenomenler,number,profile,followers,posts,platform) {
 let yeniObje={number: gelenNumber,
...
function fenomenGonderimSayisi(object,fenomenName){
  let postSayisi= 
  for(let i = 0; i < object.length; i++){
    if(fenomenName===object[i].profile)
  }
  return postSayisi;
}
...
function platformaGoreCokGonderiYapanFenomen(object,platform){
let feno;
let max=0;
for(i=0;i<object.length;i++){
  console.log(i);
  if(object[i].platform===platform){
    if (object[i].posts!== "NA" && object[i].posts>max){
      max=object[i].posts;
      feno=object[i].profile;
    }
  }
  return feno;
}
